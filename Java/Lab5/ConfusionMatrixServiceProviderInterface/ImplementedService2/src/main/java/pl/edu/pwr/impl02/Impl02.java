package pl.edu.pwr.impl02;

import ex.api.AnalysisException;
import ex.api.AnalysisService;
import ex.api.DataSet;

public class Impl02 implements AnalysisService, Runnable {
    ...
    public void calculate() throws AnalysisException{
        ...
        for(int i = 0; i < data.length; i++){
            ...
            rowsSum += truePredicted / (1.0 * allSum);
        }
        Double balancedAccuracy = rowsSum / (1.0 * data.length);
        DataSet res = new DataSet();
        res.setData(new String[][]{{"Balanced Accuracy = " + balancedAccuracy.toString()}});
        result = res;
        busy = false;
    }
