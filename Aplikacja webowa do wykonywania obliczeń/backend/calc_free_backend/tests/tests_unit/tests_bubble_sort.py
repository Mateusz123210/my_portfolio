...
class BubbleSortTestCases(TestCase):
    ...
    def test_sorting_ascending(self):
        ...
        for a in range(50):
            ...
            response = sorting.bubble_sort(numbers, "yes")
            self.assertTrue(type(response) == str, msg="Function should return string")
            temp = json.loads(response)
            self.assertTrue(type(temp) == dict, "Json should be valid")
            ...
            self.assertEquals(response_numbers, sorted(numbers), "Function should return correct result")
    ...
    def test_sorting_descending(self):
        ...
        for a in range(50):
            ...
            for i in range(number_of_numbers):
                numbers.append(random.uniform(-1000.0, 1000.0))        
            response = sorting.bubble_sort(numbers, "no")
            self.assertTrue(type(response) == str, msg="Function should return string")
            temp = json.loads(response)
            self.assertTrue(type(temp) == dict, "Json should be valid")
            self.assertTrue("solution" in  temp.keys(),"Solution should be in json")
            ...
            tmp = sorted(numbers)
            tmp.reverse()
            self.assertEquals(response_numbers, tmp, "Function should return correct result")
            self.check_hint(temp)
    ...
    def check_hint(self, temp):
        ...
        self.assertTrue(len(temp["hint"]) > 0, msg = "Hint should be valid(Lenght greater than 0)")
